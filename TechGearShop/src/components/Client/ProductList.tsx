...
export const ProductList: React.FC<Props> = ({
  ...
  return (
    <div ref={topRef} className="space-y-6 pb-16">
      ...
      {/* Header Banner */}
      <div className="p-5 md:p-6 rounded-2xl bg-slate-900 border border-slate-800 text-white shadow-md relative overflow-hidden flex flex-col justify-center transition-colors">
        <div className="relative z-10 space-y-1.5 max-w-xl">
          <span className="px-2.5 py-0.5 bg-orange-500/10 text-orange-400 border border-orange-500/20 text-[10px] font-bold rounded-md uppercase tracking-wide inline-flex items-center gap-1">
            <Layers className="w-3 h-3 text-orange-400" />
            {selectedCategory === 'all'
              ? 'TẤT CẢ SẢN PHẨM'
              : categories.find(c => c.id === selectedCategory)?.name || 'DANH MỤC'}
          </span>
          <h1 className="text-xl md:text-2xl font-bold tracking-tight">Sản Phẩm & Thiết Bị</h1>
          <p className="text-xs text-slate-400">
            Sản phẩm công nghệ chính hãng, bảo hành từ 12 - 36 tháng.
          </p>
        </div>
      </div>
      ...
      <div className="p-3.5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5">
          ...
          {/* Sort Selector */}
          <div className="relative">
            <ArrowUpDown className="w-4 h-4 absolute left-3 top-2.5 text-slate-400" />
            <select
              value={sortOption}
        ...
        <div className="pt-2.5 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
          <div className="flex items-center space-x-2 w-full sm:w-auto">
            <SlidersHorizontal className="w-3.5 h-3.5 text-orange-500" />
            <span className="font-medium text-slate-600 dark:text-slate-400">Giá lọc:</span>
